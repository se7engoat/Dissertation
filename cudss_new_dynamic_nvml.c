...
struct {
    volatile int polling_flag;
    int poll_interval_us;
    double power_sum;
    int power_samples_count;
} power_monitor = {0, 10000, 0.0, 0};


void* gpu_power_monitor(void* arg) {
    printf("[Power Monitor] Thread started, polling every %d microseconds...\n", power_monitor.poll_interval_us);
    nvmlDevice_t device = *(nvmlDevice_t*)arg;
    while (power_monitor.polling_flag) {
        unsigned int power;
        nvmlReturn_t result = nvmlDeviceGetPowerUsage(device, &power);
        if (result == NVML_SUCCESS) {
            double watts = power / 1000.0; // for milliwatts → watts divide by 1000
            printf("[Power Sample] %.2f W\n", watts);
            power_monitor.power_sum += watts;
            power_monitor.power_samples_count++;
        }
        usleep(power_monitor.poll_interval_us);
    }
    return NULL;
}
...
void benchmark_cudss(const char* mtx_path, int n_runs) {
   ...
   cudssConfigSet(handle, CUDSS_CONFIG_HYBRID_EXECUTE_MODE, &hybrid_mode, sizeof(int));
    ...
    // Decide polling interval based on nnz
    if (nnz < 10000) {
        printf("[Power Monitor] Setting polling interval to 10 ms, nnz value is: %d\n", nnz);
        power_monitor.poll_interval_us = 5000;
    } else if (nnz < 100000) {
        printf("[Power Monitor] Setting polling interval to 20 ms, nnz value is: %d\n", nnz);
        power_monitor.poll_interval_us = 10000;
    } else if (nnz < 1000000) {
        printf("[Power Monitor] Setting polling interval to 50 ms, nnz value is: %d\n", nnz);
        power_monitor.poll_interval_us = 25000;
    } else {
        power_monitor.poll_interval_us = 50000; // fallback
        printf("[Power Monitor] Setting polling interval to 100 ms, nnz value is: %d\n", nnz);
    }
    ...
    if (nvml_result != NVML_SUCCESS) {
        fprintf(stderr, "[ERROR] Failed to get NVML device handle: %s\n", nvmlErrorString(nvml_result));
        exit(EXIT_FAILURE);
    }

    //Starting pthread engine on the host to power monitor
    power_monitor.polling_flag = 1; // Start polling
    pthread_t polling_thread;
    int thread_err = pthread_create(&polling_thread, NULL, gpu_power_monitor, &device);
    if (thread_err != 0) {
        fprintf(stderr, "[ERROR] Failed to create power monitoring thread (code: %d)\n", thread_err);
        exit(EXIT_FAILURE);
    }
    printf("[Power Monitor] Started GPU power monitoring thread.\n");

    // --- Benchmark ---
    printf("\n[PHASE] ==============================================================Benchmarking phase (%d runs)==============================================================\n", n_runs);
    ...
    for (int i = 0; i < n_runs; i++) {
        ...
        // get_gpu_stats(&stats); // Update GPU stats
    ...
    printf("[POWER] Avg. Dynamic Power: %.2f W\n", 
           power_monitor.power_sum / power_monitor.power_samples_count);
